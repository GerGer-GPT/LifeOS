...
private fun PlanScreen(plan: List<DayPlanItem>, save: (List<DayPlanItem>) -> Unit, modifier: Modifier) {
    ...
    Page("План дня", "LifeOS предлагает основу — вы корректируете её по ходу дня", modifier) {
        item { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(time, { time = it }, label = { Text("Время") }, Modifier.width(105.dp), singleLine = true); OutlinedTextField(title, { title = it }, label = { Text("Новый блок") }, Modifier.weight(1f), singleLine = true) } }
